...
import { X, Check, MapPin, Gift, Users, ExternalLink } from "lucide-react";
...
export default function BrandOnboardingCard({
  brandId,
  brandSlug,
  locationCount,
  hasLoyalty,
  teamSize,
}: BrandOnboardingCardProps) {
  const storageKey = `hoptrack:brand-onboarding-${brandId}-dismissed`;
  const viewedKey = `hoptrack:brand-page-viewed-${brandId}`;
  ...
  useEffect(() => {
    ...
    setHasViewed(!!localStorage.getItem(viewedKey));
  }, [storageKey, viewedKey]);

  const steps: Step[] = [
    {
      label: "Add a second location",
      href: `/brewery-admin/brand/${brandId}/settings`,
      icon: MapPin,
      complete: locationCount >= 2,
    },
    {
      label: "Set up brand loyalty",
      href: `/brewery-admin/brand/${brandId}/loyalty`,
      icon: Gift,
      complete: hasLoyalty,
    },
    {
      label: "Invite team members",
      href: `/brewery-admin/brand/${brandId}/team`,
      icon: Users,
      complete: teamSize >= 2,
    },
    {
      label: "View your brand page",
      href: `/brand/${brandSlug}`,
      icon: ExternalLink,
      complete: hasViewed,
      external: true,
      onClickMark: viewedKey,
    },
  ];
