...
**Last Completed Sprint:** Sprint 172 — The Design Audit ✅
**Arc:** The Modernization (Sprints 171-172) — IN PROGRESS
**Last completed:** Sprint 172 — The Design Audit 🔍 ✅ — Joshua's full design audit implemented screenshot by screenshot. **Feed Cards (P0):** SessionCard redesign (brewery sub-card, grouped pour counts ×N, stats strip with duration/avg), new CheckinCard (minimal Strava-kudos-style row for 0-beer sessions), new RatingOnlyRow (compact rating without review text), RatingCard full redesign (beer-first layout, big rating, brewery·city, flavor tags, social footer), AchievementFeedCard (emoji icons replacing Lucide, DS v2 tokens). Feed diversity: achievements capped 10→3 per load. EmojiPulse social actions added to 6 card types (all 12+ now interactive). **Discover Tab:** HopRoute CTA layout flipped (map left, green NEW dot), BeerOfTheWeek big 4.8 rating + avg_rating query, Recommendation cards (style-tinted hero, glass watermark, 5-star rating), EventCard (gold accent bar, date chip, relative date, going count from event_rsvps), SeasonalBeersScroll (taller cards, style-tinted hero, info-blue NEW badge). **Profile:** Stat cards (3-card grid with amber top bars + icons, streak separated), XP bar (rank milestones Newbie→Legend, amber gradient, next rank label), Favorite Beer (amber top bar, glass thumbnail, big rating + pour count), Beer Journal (grouped entries, style accent bars, ★ rating format), Wishlist (2-col style-tinted grid, glass thumbnails), Beer Lists (cover collage, count badge, empty state CTA), Achievements (circle rings, info-blue NEW badge, card wrappers, 5-col grid), BeerDNA (style dots, sans 20px/600, premium gradient bg), PersonalityBadge (amber border, trait pills), DrinkerStatsCard (3×2 grid with hairline dividers), Breweries (monogram avatars, bigger visit count). **Brewery:** BreweryCard stat row (★/on tap/visits), MugClub premium (2px gold border, crown emoji, big price, full-width CTA, check circles), tap list SEASONAL badge blue + inline stat row, Challenge (thicker progress bar, reward sub-card). **Global:** Notifications (amber unread dot, emoji type icons), Search (glass thumbnails, type badges, amber query highlighting). 2 new files (CheckinCard, RatingOnlyRow), ~35 modified, 0 migrations, 0 new type errors. Build clean. KNOWN: Empty.
**Previous Arc:** The Premium (Sprints 167-170) — COMPLETE (arc closer shipped)
**Last completed:** Sprint 170 — The Glass 🥃 ✅ — Premium Arc closer. Four-track sprint — zero migrations, zero new API routes. **Track 1 (Motion Normalization):** 7 `framer-motion` → `motion/react` import stragglers fixed (S157 migration complete). 2 new presets added to `lib/animation.ts` (`slideUpSmall`, `slideInRight`). ~21 files normalized: inline springs → preset references across celebrations, feed cards, UI, profile. **Track 2 (Detent Sheet):** New `useDetentSheet` hook — 3-detent snap logic (peek/half/full), velocity-based fling detection, haptic feedback, `useReducedMotion` support. New `DetentSheet` component replaces MinimizedSessionBar + TapWallSheet with unified iOS-style bottom sheet. Peek: minimized bar (brewery + beer count + time). Half: session summary + quick actions (Pours/Styles/Time stats grid, Log Beer + End buttons). Full: complete TapWall content (header + beer list + session tray). `TapWallMode` type expanded from `'closed'|'open'|'minimized'` to `'closed'|'peek'|'half'|'full'`. AppShell rewired from 3 separate session blocks to 1 DetentSheet render. **Track 3 (OLED Black Mode):** Full `[data-theme="oled"]` CSS variable block in globals.css — true `#000000` bg, neutral-cool surface scale (`#0A0A0A`/`#141414`/`#1E1E1E`), border-glow elevation (shadows invisible on true black, replaced with `0 0 0 1px rgba(255,255,255,0.06)`). ThemeProvider extended: `Theme = "dark"|"light"|"oled"`, cycle toggle + direct `setTheme()`. ThemeToggle: 3-way segmented selector (Sun/Moon/Smartphone icons) with gold `layoutId` sliding indicator. OLED is opt-in only (system `prefers-color-scheme: dark` maps to "dark", not "oled"). **Track 4 (Arc Close):** 4 new test files, 79 new tests (1765→1844): board-settings (22, deferred S167), notification-categories (10, deferred S168), useDetentSheet (25), theme-toggle (22). 8 pre-existing CI lint errors fixed (conditional hooks in FourFavorites/PersonalityBadge, prefer-const, ban-types, no-var, React compiler warnings). Dead code: MinimizedSessionBar.tsx + TapWallSheet.tsx have zero imports (candidates for deletion). 6 new files (useDetentSheet hook, DetentSheet component, 4 test files), ~50 modified, 0 migrations, 0 new API routes, 1844 tests (all pass), 0 lint errors. Build clean. KNOWN: Empty. **Premium Arc CLOSED.**
**Retro (170):** `docs/retros/sprint-170-retro.md` (facilitated by Sage)
**Previous Arc:** The Premium (Sprints 167-170) — COMPLETE (arc closer shipped)
**Last completed:** Sprint 169 — The Details 🔍 ✅ — Premium Arc track 3. Five-track visual depth sprint — zero migrations, zero new API routes. **Track 1 (Leaderboard Podium):** New `LeaderboardPodium.tsx` — horizontal top-3 cards with height hierarchy (2nd|1st|3rd), Crown icon on #1, staggered award ceremony animation (3rd→1st→2nd). localStorage rank snapshots per category/scope/timeRange compute deltas on next load. Existing `LeaderboardRow` `change` display activated. Entries 4+ render below podium. **Track 2 (Achievement Rarity + Progress Rings):** SVG `stroke-dasharray` progress rings on locked badges with tier-colored animation. Rarity computed server-side (unlock count / total users). 4-tier labels: Legendary (<5%), Rare (5-20%), Uncommon (20-50%), Common (>50%). Color-coded rarity pill in detail modal. **Track 3 (Beer List Mosaics + Stats):** Queries expanded with `cover_image_url`, `item_type`, `brewery(id, name)`. 2x2 preview mosaic on list home cards (gradient fallback). 40x40 thumbnails on every beer item. Stats summary line (N beers · N styles · N breweries · avg). PillTabs segmented view toggle (List/Mosaic) on detail page. Mosaic: aspect-square cards with position badges, gradient overlays, hover-to-gold. **Track 4 (Your Round WoW):** Parallel dual fetch (this week + previous week) via `computeYourRoundRange`. Delta indicators on Numbers slide StatCells (+N green / -N red / same gray). TrendingUp/TrendingDown icons. Contextual trend messages. **Track 5 (Haptic Audit):** StarRating + ReactionBar migrated from raw `navigator.vibrate` → `useHaptic`. Haptics added to Button (press), IconButton (tap), WishlistButton (success/tap), FollowBreweryButton (success/tap), ThemeToggle (selection), Modal close (tap), usePullToRefresh threshold (selection). New `microInteraction` presets in `lib/animation.ts`. Test setup: `window.matchMedia` mock for haptic-dependent components. 1 new file (LeaderboardPodium.tsx), ~18 modified, 0 migrations, 0 new API routes, 1765 tests (all pass), 0 lint errors. Build clean. KNOWN: Empty.
**Retro (169):** `docs/retros/sprint-169-retro.md` (facilitated by Sage)
**Last completed:** Sprint 168 — The Pages 📄 ✅ — Premium Arc track 2. Page-level visual refinement across 5 consumer pages, zero migrations, zero APIs. **Track 0 (Cleanup):** Deleted BoardTapList.tsx dead file (417 lines, zero imports since S167). Fixed `framer-motion` → `motion/react` imports across 7 recap files (S157 migration miss). Extracted shared `RecapSectionTitle` from 4 duplicated definitions → `components/session/recap/RecapSectionTitle.tsx`. **Track 1 (Beer Detail Hero):** Hero image h-40 → h-56 (40% taller), two-layer gradient overlay (bottom fade + top vignette), deeper content overlap (-mt-8 → -mt-12), metadata stat pills with surface-2 backing, gold glow drop-shadow on 4.5+ rated beers, description in surface-2 card. **Track 2 (Search Results):** 40x40 gradient swatch thumbnails with first-initial overlay (generateGradientFromString), beers grouped by brewery when 2+ share same brewery, 40x40 brewery icons with MapPin, count badges on section labels, 150ms hover transition. Shared by GlobalSearch + ExploreClient. **Track 3 (Notifications):** PillTabs filter bar (pill variant, sm) with All/Social/Achievements/Rewards/System categories. 16 notification types mapped to 4 categories via CATEGORY_MAP. Unread count badges per category. Time section headers (Today/Yesterday/This Week/Older) with sticky backdrop blur. EmptyState per category. Mark-all-read → compact CheckCheck icon. **Track 4 (Settings):** Raw Section → Card component. 4 group headings (Personal/Preferences/Social/Account) in font-mono uppercase tracking-widest. Tightened intra-group spacing. Icon backing with surface-2 + rounded-xl. Danger section preserved. **Track 5 (Session Recap):** Stats grid 1x4 → 2x2 mobile with 32px gold beer count. Beer log zig-zag (glass icon alternates left/right, entry animations from alternating sides). NEW badge on first-time beers (gold pill). Single achievement → full-width hero treatment (56px icon, scale-in). Brewery rating gold radial gradient + blur-in. Share CTA gold glow shadow. ~16 files modified, 1 created (RecapSectionTitle.tsx), 1 deleted (BoardTapList.tsx), 0 migrations, 1765 tests (unchanged), 0 lint errors. Build clean. KNOWN: Empty.
**Retro (168):** `docs/retros/sprint-168-retro.md` (facilitated by Sage)
**Last completed:** Sprint 167 — The Board 🎬 ✅ — Premium Arc opener. 5 selectable Board display formats, zero migrations, zero APIs. **Format 1 (Classic):** Existing vertical auto-scroll list refactored from BoardTapList into BoardClassic module with shared component extraction (GlassIllustration, SizeChips, BeerMetaRow, BeerStatsRow, groupBeersByType, deriveBeerLists into BoardShared.tsx). useAutoScroll hook extracted. **Format 2 (Grid):** 2-3 column responsive card layout with style-tinted backgrounds (IPA green, Stout espresso via getStyleFamily). Glass art prominent. Featured beer full-width. **Format 3 (Compact):** Maximum density 2-column grid, name + price only. FORMAT_FORCED auto-overrides settings (showGlass/showDesc/showRating/showStats forced false). **Format 4 (Poster):** Hero featured beer (40vh) with large glass SVG + style-colored radial gradient. Hero rotates every 15s with AnimatePresence crossfade. Compact list below with auto-scroll. Dot indicators. **Format 5 (Slideshow):** Full-screen cinematic per-beer spotlight. Auto-cycles 4.5s with AnimatePresence crossfade. Ambient mesh gradient background shifts by beer style via CSS transition. Pause on hover/tap (resume after 3s). Progress bar (gold fill) for 15+ beers, dot indicators for fewer. Brewery name top-left, HopTrack badge bottom-right. Slideshow hides header/footer, gets own mini settings gear. **Architecture:** BoardDisplayFormat type, FORMAT_DEFAULTS (recommended), FORMAT_FORCED (hard overrides), getEffectiveSettings() helper, FORMAT_LABELS. FormatRenderer switches in BoardClient. PillTabs-style format selector in BoardHeader settings panel. Forced toggles dimmed. Slideshow hides fontSize. 7 files created, 3 modified, 0 migrations, 1765 tests (unchanged), 0 lint errors. Build clean. KNOWN: Empty (pre-existing Turbopack panic unchanged). BoardTapList.tsx dead file — cleanup P2.
**Retro (167):** `docs/retros/sprint-167-retro.md` (facilitated by Sage)
**Previous Arc:** The Glow-Up (Sprints 163-166) — COMPLETE (arc closer shipped)
**Last completed:** Sprint 166 — The Finish 🏁 ✅ — Glow-Up Arc closer. Pure visual elevation — zero features, zero migrations, zero APIs. **Track 1 (Consumer Polish):** Deleted orphaned FeedTabBar.tsx (57 lines, zero imports). Beer detail: 4 raw divs → Card (header elevated, similar beers flat+hoverable, activity log, empty state). Fixed hardcoded `from-[#1C1A16]` → `from-[var(--bg)]` for light mode. Achievements: progress bar → Card with card-bg-stats. Beer lists: create form, empty state, list items → Card with card-bg-collection. Light mode audit clean. **Track 2 (Brewery Admin Dashboard Lift):** Today's Snapshot → Card. 8 KPI cards → Card spacious (shadow system on every metric). Staff Quick Action → Card hoverable. Top Beers → Card flat. Recent Visits → Card flat compact. Quick Actions grid → Card hoverable flat. Empty sessions, All-Time Stats, Loyalty → Card. ROIDashboardCard (2 variants) → Card with card-bg-stats. PosDashboardCard (3 branches) → Card. RecentActivityFeed empty → Card. 8 files modified, 1 deleted, 0 new files, 0 migrations, 1765 tests (unchanged), 0 lint errors. Build clean. KNOWN: Empty. **Glow-Up Arc CLOSED.**
**Retro (166):** `docs/retros/sprint-166-retro.md` (facilitated by Sage)
**Last completed:** Sprint 165 — The Surfaces 🪟 ✅ — Page-level visual elevation across 5 consumer surfaces. Zero features, zero migrations, zero new APIs — applied S163/S164 design system. **Track 1 (Home FeedTabBar → PillTabs):** Replaced 57-line bespoke FeedTabBar with PillTabs underline variant. Free haptics, roving tabindex keyboard nav, reduced-motion support. FeedTab type re-exported from HomeFeed.tsx. **Track 2 (Friends PillTabs + Shadow Elevation):** Two PillTabs migrations: segmented variant for My Friends/Leaderboards primary tabs, pill variant for leaderboard sub-tabs (Sessions/Unique Beers/Breweries). Shadow system applied to search input, search results, friend rows, pending/sent request cards. card-bg-featured on empty state with relative z-10 for pseudo-element layering. Hover elevation on interactive rows. **Track 3 (Explore Card Adoption + Shadows):** Shadow on view toggle, filter panel, NearMeCard. Hover elevation on brewery cards. Fade-edge masks (`mask-image: linear-gradient(...)`) on Near Me + Recently Visited horizontal scroll containers. **Track 4 (You Tab Hero + CTA + Shadow Pass):** Shadow-elevated on hero card. Shadow + hover on Your Round/Wrapped CTAs. Shadow pass across stats card, activity heatmap, achievements, wishlist items, brewery passport, past HopRoutes. Activity divider upgraded from h-px to border-t with opacity-60. **Track 5 (Profile Hero + Sticky Tab):** Shadow-elevated on hero banner. Shadow-card on sticky ProfileTabs navigation via className prop. 6 files modified, 0 new files, 0 migrations, 1765 tests (unchanged), 0 new lint errors. Build clean. KNOWN: Empty.
**Retro (165):** `docs/retros/sprint-165-retro.md` (facilitated by Sage)
