...
| BL-007 | Tier feature matrix & billing clarity | Define exactly what each paid tier (Tap/Cask/Barrel) gets — feature-by-feature matrix. Show current tier prominently on billing page with clear upgrade/downgrade options. Multi-location pricing model ($149 base + $29-49/location from REQ-072) needs to be surfaced. | Sprint 95 |
| BL-008 | Billing page — show current tier + options | Billing page should show your current tier clearly, what you're paying, what features you have, and what upgrading unlocks. Currently just Stripe portal link — needs to be a proper tier comparison. | Sprint 95 |
| BL-009 | Mug club consumer experience | How do users see mug clubs on brewery profiles? Do members get a badge on customer lists? Show club memberships on user profiles? Need full consumer-facing mug club UX. | Sprint 95 |
| BL-010 | Perk/reward fraud prevention | Cross-cutting security issue across loyalty, mug clubs, events, promotions, and HopRoute incentives. Users can currently self-report sessions without verification — need brewery-side confirmation (QR scan, staff approval, or POS validation) before perks are awarded. Prevents fake sessions for free rewards. | Sprint 95 |
| BL-011 | Session drawer UX overhaul | Beer selections don't persist when closing drawer. Need: (1) save selections when drawer closes, (2) lower drawer with selections intact, (3) active session card showing live status — current beer, total beers, HopRoute status, drinking companions. (4) Cancel session option with "are you sure" confirmation + data loss warning (currently only "end session" which saves). Match app look and feel. | Sprint 95 |
| BL-012 | Homepage screenshot refresh | Marketing homepage (`/for-breweries` + landing) has outdated screenshots. Need fresh screenshots from current app for both consumer and brewery admin views — highlight key features. | Sprint 95 |
