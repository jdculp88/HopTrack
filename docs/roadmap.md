...
| # | Item | Notes | Source |
|---|------|-------|--------|
| BL-001 | Deduplication logic (beers + breweries) | SHIPPED (S98) — Migration 067: unique index on `(brewery_id, LOWER(TRIM(name)))` for active beers, `(LOWER(TRIM(name)), city, state)` for breweries. Existing dupes cleaned. | Sprint 78 |
...
| BL-003 | Incomplete beer data handling | SHIPPED (S98) — `IncompleteBeerBadge` component shows "Needs details" on beers with suspect ABV or missing style. Wired into BeerCard. | Sprint 78 |
| BL-004 | Lint cleanup pass | SHIPPED (S94) — Lint errors driven to 0 during Flywheel arc. | Sprint 78 |
| BL-005 | Menu upload save fails (PGRST204) | SHIPPED (S94) — Migration 065 forced PostgREST schema cache reload. Issue resolved. | Sprint 84 |
...
| BL-007 | Tier feature matrix & billing clarity | SHIPPED (S96) — 20-feature x 4-tier FEATURE_MATRIX in `lib/stripe.ts`. Free tier card, current tier badge, upgrade/downgrade CTAs. | Sprint 95 |
| BL-008 | Billing page — show current tier + options | SHIPPED (S96) — Shipped with BL-007. | Sprint 95 |
...
| BL-010 | Perk/reward fraud prevention | SHIPPED Phase 1 (S96) — Redemption codes (6-char, 5-min expiry, staff confirmation). Phases 2-3 per `docs/plans/fraud-prevention-design.md`. | Sprint 95 |
| BL-011 | Session drawer UX overhaul | SHIPPED (S96) — SessionContext, minimized mode, cancel session with hard-delete. | Sprint 95 |
