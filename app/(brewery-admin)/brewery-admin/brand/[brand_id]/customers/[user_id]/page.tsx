...
import { BrandCustomerProfileClient } from "./BrandCustomerProfileClient";

export async function generateMetadata({ params }: { params: Promise<{ brand_id: string; user_id: string }> }) {
  const { user_id } = await params;
  const supabase = await createClient();
  const { data: profile } = await supabase.from("profiles").select("display_name").eq("id", user_id).single() as any;
  return { title: `${(profile as any)?.display_name ?? "Customer"} — Brand Customer Profile` };
}

export default async function BrandCustomerProfilePage({ params }: { params: Promise<{ brand_id: string; user_id: string }> }) {
  ...
  if (locationIds.length === 0) redirect(`/brewery-admin/brand/${brand_id}/customers`);
  ...
  if (!profile) redirect(`/brewery-admin/brand/${brand_id}/customers`);
  ...
  if (!sessions || sessions.length === 0) redirect(`/brewery-admin/brand/${brand_id}/customers`);
  ...
  // Enrich journey timeline with per-session beer data
