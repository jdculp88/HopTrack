...
export default async function BrandReportsPage({ params }: { params: Promise<{ brand_id: string }> }) {
  ...
  if (locationIds.length > 0) {
    ...
    const [
      ...
      supabase.from("sessions").select("id, user_id, brewery_id, started_at").in("brewery_id", locationIds).eq("is_active", false).gte("started_at", rangeStart) as any,
      supabase.from("sessions").select("id, user_id, brewery_id").in("brewery_id", locationIds).eq("is_active", false).gte("started_at", prevRangeStart).lt("started_at", rangeStart) as any,
      supabase.from("beer_logs").select("id, beer_id, rating, quantity, brewery_id, beer:beers(name, style)").in("brewery_id", locationIds).gte("logged_at", rangeStart) as any,
      supabase.from("brewery_followers").select("id, brewery_id").in("brewery_id", locationIds) as any,
    ...
    if (programIds.length > 0) {
      const { data: redemptions } = await (supabase.from("loyalty_redemptions").select("id, program_id").in("program_id", programIds).gte("redeemed_at", rangeStart) as any);
