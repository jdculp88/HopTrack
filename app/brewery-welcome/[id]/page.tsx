...
import { getCachedBreweryWelcomeData, getCachedBreweryWelcomeMetadata } from "@/lib/cached-data";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const meta = await getCachedBreweryWelcomeMetadata(id);
  if (!meta) return { title: "Brewery — HopTrack" };

  const { brewery, beerCount } = meta;
...
export default async function BreweryWelcomePage({ params }: Props) {
  const { id } = await params;
  const { brewery, beers, events } = await getCachedBreweryWelcomeData(id);
